...
class AuthitemController extends SBaseController {
  ...
  public function actionAssign() {
    ...
    $data['roleAssignedRoles'] = array();
    $data['roleNotAssignedRoles'] = array();
    ...
    $assData = isset($_POST['Assignments']['data']) ?
      $_POST['Assignments']['data'] : array('assignInAdmin'=>true);
    ...
    $assignRolesToRole = Yii::app()->request->getParam('assignRolesToRole', 0);
    $revokeRolesToRole = Yii::app()->request->getParam('revokeRolesToRole', 0);
    ...
    } else if ($assignRolesToRole && is_array($authItemAssignName)) {
      $this->_assignChild($authItemName, $authItemAssignName);
      $this->_setMessage(Helper::translate('srbac', 'Role(s) Assigned'));
    } else if ($revokeRolesToRole && is_array($authItemRevokeName)) {
    ...
    if (!Yii::app()->request->isAjaxRequest) {
      ...
      if ($userid != "") {
        ...
        $this->_getTheOpers();
      } else if ($assignRolesToRole != 0 || $revokeRolesToRole != 0) {
        $this->_getTheRolesToRoles();
      }
    }
  }
  ...
  //CVarDumper::dump($_POST, 5, true);
    ...
    /* @var $auth CDbAuthManager */    
    //If not ajax show the assign page    
    $this->renderPartial('tabViews/roleToOneUser', array(
          'model' => $model,
          'message' => $this->_getMessage(),
          'userid' => $userid,
          'data' => $data
   ...
   * roles to user tab
   */
  public function actionGetRolesToRoles() {
    $this->_setMessage("");
    $this->_getTheRolesToRoles();
  }
  
  public function actionGetRolesToRolesList() {
	$this->renderPartial('tabViews/roleToRoleList', array(),false, true);
  }

  /**
   * Gets the assigned and not assigned roles of the selected user
   */
  private function _getTheRolesToRoles() {
    $model = new AuthItem();
    $name = isset($_POST["AuthItem"]["name"][0]) ? $_POST["AuthItem"]["name"][0] : "";
    $data['roleAssignedRoles'] = Helper::getRoleAssignedRoles($name);
    $data['roleNotAssignedRoles'] = Helper::getRoleNotAssignedRoles($name);
    if ($data['roleAssignedRoles'] == array()) {
      $data['revoke'] = array("name" => "revokeRolesToRole", "disabled" => true);
    } else {
      $data['revoke'] = array("name" => "revokeRolesToRole");
    }
    if ($data['roleNotAssignedRoles'] == array()) {
      $data['assign'] = array("name" => "assignRolesToRole", "disabled" => true);
    } else {
      $data['assign'] = array("name" => "assignRolesToRole");
    }
